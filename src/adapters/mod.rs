...
// ── Lifecycle boilerplate macros ──────────────────────────────────

/// Generate `CognitiveModule` lifecycle methods (`on_mount`, `on_unmount`, `state`).
///
/// Usage inside `impl CognitiveModule for MyAdapter { ... }`:
/// ```ignore
/// adapter_lifecycle!();
/// ```
macro_rules! adapter_lifecycle {
    () => {
        fn on_mount(&mut self) {
...
/// Like `adapter_lifecycle!()` but omits `on_unmount` — use when the
/// adapter has a custom unmount that clears extra state.
macro_rules! adapter_lifecycle_no_unmount {
    () => {
        fn on_mount(&mut self) {
            self.state = ModuleState::Idle;
        }
...
pub(crate) use adapter_lifecycle;
pub(crate) use adapter_lifecycle_no_unmount;
