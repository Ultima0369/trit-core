//! Core interrupt types — conflict detection, policy violation, and meta-monitoring.
//!
//! These types are core-level data structures: they carry frame-pair information
//! produced by [`TernaryAlgebra`](super::TernaryAlgebra) cross-frame operations,
//! classify conflict types, and provide a ring-buffer monitor for invariants.
//! No policy logic lives here — that belongs in the `meta` layer.
//!
//! Moved from `meta::interrupt` in the Layer Dependency Cleanup (2026-07-08):
//! these are core data types consumed by `algebra`, `decision_engine`, and
//! `safe_fallback` — they don't belong above `core` in the dependency stack.
