...
impl DecisionEngine {
    ...
    /// # Priority rules (in order of application)
    ///
    /// | Stage | Rule | Overridable by |
    /// |-------|------|---------------|
    /// | 2a | FirstPerson > Science … in non-dangerous domains | Domain gate (Physical/Engineering skip this) |
    /// | 2b | Domain-specific frame priority (e.g. Science in Physical) | — |
    /// | 2c | `ForceCollapse` when domain-required frame absent | Stage 4 |
    /// | 3 | Reflexive guard: override computable → Hold if alert fires | User disabling reflexive |
    /// | 4 | **SafeFallback** (last word): Hold/Unknown in dangerous domain → False | SafeFallback::enabled flag |
    ///
    /// **Stage 4 (SafeFallback) always has the final say** in dangerous domains
    /// (Physical/Engineering). If Stage 2 preserved a FirstPerson True, but the
    /// result is Hold with cross-frame interrupts, SafeFallback forces False.
    /// This is intentional: in dangerous domains, physical reality overrides
    /// subjective experience at the safety gate.
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    #[test]
    fn first_person_priority_skipped_in_dangerous_domains() {
        // Physical domain: FirstPerson should NOT override Science.
        // A person saying "I feel the bridge is safe" does not override
        // structural load calculations.
        //
        // When cross-frame inputs (FirstPerson + Science) go through t_and_n,
        // they produce Hold/Meta. In Physical domain the policy tries to commit
        // Science, but resolve_arbitrated_word sees the t_and_n result was Hold
        // (cross-frame conflict) and preserves Hold/Meta so the conflict is not
        // silently overridden. SafeFallback then forces False for the Hold in
        // the dangerous domain.
        let mut engine = DecisionEngine::new();
        let trits = vec![
            word(Frame::FirstPerson, TritValue::True, 0.9),
            word(Frame::Science, TritValue::False, 0.8),
        ];
        let result = engine.decide(&trits, &Domain::Physical).unwrap();
        // SafeFallback fires: Hold in dangerous domain → forced False
        assert_eq!(result.final_word.value(), TritValue::False);
        assert!(result.safe_fallback_triggered);
        // The arbitration side DID try to commit Science
        assert!(matches!(result.policy_action, ArbitrationResult::Commit(_)));
    }

    #[test]
    fn first_person_priority_preserved_in_non_dangerous_domain() {
        // General domain: FirstPerson IS preserved over Science.
        // Lived experience should not be silently overridden by statistics.
        let mut engine = DecisionEngine::new();
        let trits = vec![
            word(Frame::FirstPerson, TritValue::False, 0.3),
            word(Frame::Science, TritValue::True, 0.7),
        ];
        let result = engine.decide(&trits, &Domain::General).unwrap();
        // FirstPerson False preserved over Science True
        assert_eq!(result.final_word.value(), TritValue::False);
        assert_eq!(result.final_word.frame(), Frame::FirstPerson);
    }
}
