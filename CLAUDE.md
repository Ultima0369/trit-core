...
> **代码架构 vs 认知愿景**：以下五层是**代码实现栈**。另有一套 **L0-L5 认知层级愿景模型**（`aurora/00_manifest/COGNITIVE_ARCHITECTURE_LAYERS.md`），描述认知能力从协议运算到"照见"的远期递进——那是方向，非当前规格。两套"层"编号相同但语义不同，勿混淆。
...
- **`aurora/src/ingest/`**: JSON file ingestion — `JsonFallbackSource` reads a contact JSON array (no trait layer; one implementation).
...
### Aurora-Specific
- **BC stores are concrete** — `SqliteAuditLog` and `SqliteAnnotationStore` are used directly (no trait abstraction). Both expose owned-returning methods (`query_owned()` / `get_contact_owned()`) for SQLite compatibility.
