...
|---|---|---|---|
| 通信方式 | ResonanceBus（内存） | TCP（网络） | TCP + 自动拓扑 |
| 节点发现 | 手动注册 | TCP 连接 | 种子引导 + HEARTBEAT 交换 |
| 并发模型 | 单线程 | tokio 多线程 | tokio 多线程 |
| 适用范围 | 本地模拟、测试 | 真实分布式部署 | 动态集群、Docker Compose |
| 内存上限 | 256 节点、10K 消息 | 无硬性上限 | 无硬性上限 |
