...
性能目标为 10,000 TPS（每秒 TritWord 操作）。端到端基准测试（`docs/reports/performance-validation.md`）已验证：
...
- dhat 堆分析已完成：热路径零分配已验证（`src/bin/dhat_profile.rs`），冷路径仅 MetaInterrupt 分配
- 多线程并发负载测试已完成（`tests/concurrency_test.rs`，7 个测试，305 总测试通过）
- 无大规模集群性能数据（>3 节点）
...
安全审计（`docs/reports/security-audit.md`）是自审计——由项目开发者执行，非独立第三方。
...
- API 文档（`docs/reference/api.md`）可能过时（需与源码同步）
...
如果你在某个局限领域有专长（形式化方法、分布式系统、安全审计），欢迎贡献。见 [CONTRIBUTING](../../how-to/CONTRIBUTING.md)。
