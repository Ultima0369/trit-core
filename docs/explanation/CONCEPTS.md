...
pub enum Frame {
    ...
    Meta,          // 冲突仲裁/策略输出（系统内部使用，不允许作为外部输入）
    FirstPerson,   // 第一人称主观报告
    Embodied,      // 身体/生理状态参考系
    Relational,    // 关系/社会互动参考系
    GeoEco,        // 地理与生态参考系（出生地/气候/生物群落）——长见识输入的落点
    Developmental, // 成长轨迹参考系（人生阶段/技能路径）
    Role,          // 角色参考系（社会/职业角色）
    Environmental, // 环境状态参考系（噪声/光照/天气/社交密度）
}
```
...
**第一人称优先**：当 `FirstPerson` 与 `Science` 冲突时，默认 Preserve `FirstPerson`。这是为了避免科学框架覆盖主观事实（例如患者疼痛报告与“检查无异常”之间的冲突）。

**心智工程参考系**：`Embodied` 与 `Relational` 不自带域级优先规则，但可被注意力调度器与自我知识模型用作输入来源。
...
pub enum Domain {
    ...
    Organizational,  // 组织决策：多 Frame 跨角色协商
    Relational,      // 关系决策：优先 Relational 帧
    Cognitive,       // 认知决策：优先 Embodied 信号
    Environmental,   // 环境适应：优先 GeoEco 帧
}
```
...
| Organizational | 多 Frame 协商 | 否 | 跨角色/流程协商，不强制坍缩 |
...
```rust
pub struct TritWord {
    value: TritValue,  // 三值状态（私有，通过构造器/访问器使用）
    phase: Phase,      // 连续倾向度（私有）
    frame: Frame,      // 决策域（私有）
}
```
...
实验结果（20 个英语场景，详见 [validation-report](../reports/validation-report.md)）：
- 40% 的案例（8/20）：二元基线覆盖或平滑了真实冲突
- 100% 的 ValueJudgment 案例：二元无法表达"算法不应该决定这个"
- 100% 的 MedicalEthics 案例：二元忽略患者特定上下文
- 所有跨帧冲突场景：二元无法表达 Hold 状态
