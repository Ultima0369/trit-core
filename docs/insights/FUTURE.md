...
`net/` 模块实现了完整的消息类型、状态机和 PLL 控制器，但所有通信通过内存中的 `ResonanceBus` 进行——没有真实的网络传输。
...
- 无网络分区容错
- 无拜占庭容错
- PLL 参数（kp=0.3、deadband=0.05）仅在内存模拟中验证
...
性能目标为 10,000 TPS（每秒 TritWord 操作）。热路径微基准测试显示 ~3ns/op，理论上远超目标。但端到端性能（含 JSON I/O）未经验证。
...
- `criterion` 基准测试扩展为端到端场景
- `dhat` 堆分析验证零分配热路径
- 多线程下的 `ResonanceBus` 并发测试
