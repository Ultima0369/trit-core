...
方法：
- `fn new(v: f64) -> Result<Phase, PhaseError>` — 严格构造，越界/非有限返回 Err
- `fn new_clamped(v: f64) -> Phase` — 静默钳制到 `[0.0, 1.0]`
- `const fn neutral() -> Phase` — 常量 0.5
- `const fn full_true() -> Phase` — 常量 1.0
- `const fn full_false() -> Phase` — 常量 0.0
- `fn inner(self) -> f64`
...
- `fn t_sense(phase: f64, frame: Frame) -> Result<TritWord, PhaseError>`
- `fn t_sense_clamped(phase: f64, frame: Frame) -> TritWord`
