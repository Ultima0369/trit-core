...
| 401 | 未授权（未来 M3 局域网同步部署预留） | `{ "error": { "code": "UNAUTHORIZED", "message": "Valid session required", "trace_id": "uuid-v4" } }` |
...
*本架构设计文档为 Aurora 个人版（M0-M1）技术方案阶段产出。所有技术决策以 CHARTER.md 为最高判据。分布式接口预留但本期不实现。*
```
...
*此文件为项目开机导航。每次重大决策或阶段完成后更新"当前进度"和"上次决策"。*
```
