...
*本文档为参考系建模模块的详细规格。v0.2.0 修正了 `self_weight` 为 `individual_weight`（与 Trit-Core v0.3.0 的 `Frame::Individual` 对齐），移除了 wrapper 定义（改为直接扩展 enum），增加了角色边界预警的系统响应（Awareness 原则：只通知，不阻止）。不是指教，是提醒。*
