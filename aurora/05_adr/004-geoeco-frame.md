...
> **已实现**。`src/core/frame.rs` 的 `Frame` enum 现含 12 个变体（基础 8 + 本 ADR 扩展 4）；`src/meta/domain.rs` 的 `Domain` enum 现含 10 个变体（基础 6 + 本 ADR 扩展 4）。本 ADR 描述的扩展已落地为代码，非待办。
...
*本 ADR 记录 Aurora 扩展 Trit-Core Frame 系统的决策过程。*
