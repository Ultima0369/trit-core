...
> ⚠️ **当前实现状态标注**：本文档为**远期设计草案，未启动**。RLHF / 微调 / 损失函数（$L_{frame}$ 等）通篇为**设想**，与 `docs/NARRATIVE_CHARTER.md` 底线 3"不进化"存在张力。底线 3 规定：不基于用户数据在线学习、不个性化、不画像。即便未来启动，**RLHF 仅限全局脱敏反馈，禁用个体数据**（见 CHARTER 底线 3"不进化"）。**当前未实现任何本地模型训练**，trit-core / aurora 均无训练管线、无损失函数代码、无 RLHF 循环。文中下述设计内容保留作设计参考，不代表现状。
...
> ⚠️ **愿景设计（未实现）**：本节损失函数（$L_{frame}$、$L_{phase}$、$L_{hold}$、$L_{conflict}$、$L_{survival}$）均为**设想**。当前 trit-core / aurora 代码库无任何训练管线或损失函数实现。启动时须先解决与 `docs/NARRATIVE_CHARTER.md` 底线 3"不进化"的张力。
...
- 输入：一段对话/行为序列
- 输出：参考系概率分布（`Frame` 共 12 变体：Science、Individual、Consensus、Absolute、Meta、FirstPerson、Embodied、Relational、GeoEco、Developmental、Role、Environmental — 见 `src/core/frame.rs`；外部输入不含 Meta）
...
> ⚠️ **愿景设计（未实现）**：下述超参数为设想值，无对应代码。当前无训练循环。
...
> ⚠️ **愿景设计（未实现）**：本节 RLHF / 三值反馈 / 对抗训练均为**设想**，与 `docs/NARRATIVE_CHARTER.md` 底线 3"不进化"存在张力。底线 3 禁止基于用户数据在线学习。若未来启动，**RLHF 仅限全局脱敏反馈，禁用个体数据**——任何反馈聚合须先脱敏，不可回溯到个体用户、不可用于个体画像。当前代码库无 RLHF 实现。
...
> ⚠️ **愿景设计（未实现）**：本节规模、量化、推理优化均为设想，当前无任何本地模型集成。
...
> ⚠️ **愿景接口（未实现）**：下列 `LocalModelPerception`、`SurvivalRisk`、`LocalModel` trait 均为**设想接口**，当前代码库不存在。`Frame`、`Phase`、`MetaInterrupt` 为真实类型（见 `src/core/frame.rs`、`src/core/trit_word.rs`、`src/meta/interrupt.rs`）。`SurvivalRisk` 为概念性伪代码类型，非现有类型。
...
用户对话 → 本地小模型（perceive）
    ...
    ▼                   ▼
冲突检测？           生存风险？
├─→ 是 → MetaInterrupt  ├─→ High → SecurityMode::Refusal
│  （FrameMismatch）    │         + SafeFallback::guard()
│                     │         （强制 TritValue::False + Phase::full_false()）
│                     │
...
> ⚠️ **愿景清单（未实现）**：下列验证项针对未启动的训练/集成设计。注意 8.2 中 `FrameRegistry` 非现有类型（概念性）；`TernaryAlgebra::cross_frame_conflict` 为现有内部方法（见 `src/core/algebra.rs`），非公开注册接口。
...
*本文档为本地小模型与 trit-core 的伦理集成规格。所有设计决策基于 FIRST_PRINCIPLES.md 的五公理。训练目标函数、损失权重、RLHF 策略均已明确，接受审查。用户自负其责。*
