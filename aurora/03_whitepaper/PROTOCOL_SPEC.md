...
*本白皮书为 Aurora 协议的技术规格。v0.2.0 修正了 Frame 扩展方式（直接扩展 enum，非 wrapper）、毁灭级冲击响应（Awareness 替代 SafeMode）、加密方案（SQLCipher 替代 AES-256-GCM）、角色边界红色预警（用户可覆盖）。所有扩展的 Frame/Domain 在 Trit-Core v0.3.0 基础上进行向后兼容的扩展。*
