//! SQLite-backed audit log for the AuditTrail BC.
//!
//! Audit entries are append-only — rows can be inserted but never updated or deleted.

use crate::bc::audit_trail::{
    AuditDecisionSnapshot, AuditEntry, AuditEventType, AuditFilter, ContactAuditRecord,
    OverrideRecord,
};
...
impl SqliteAuditLog<'_> {
    ...
    pub fn query_owned(&self, filter: &AuditFilter) -> Result<Vec<AuditEntry>, BcError> {
                                ...
                                arr.iter()
                                    .filter_map(|v| {
                                        Some(ContactAuditRecord {
                                            ...
                                            trit_value: v.get("trit_value")?.as_str()?.to_string(),
                                        })
                                    })
                                    .collect()
                            })
...
mod tests {
    ...
    fn audit_log_is_append_only() {
        ...
        // Trying to modify an existing entry is impossible through the API —
    ...
    fn entry_with_contact_participation_roundtrips() {
        ...
        let snapshot = AuditDecisionSnapshot {
            signal_count: 3,
            signal_frames: vec!["Embodied".into(), "Individual".into(), "Science".into()],
            result_value: "Hold".into(),
            result_frame: "Meta".into(),
