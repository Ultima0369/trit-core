...
//!
//! # Port
//! [`RenderPort`] trait — the single interface for presentation rendering.
...
pub struct RadarPoint {
    /// Frame name (axis label).
    pub frame: String,
    /// Phase value [0.0, 1.0].
    pub phase: f64,
}

/// A radar chart showing frame weights.
#[derive(Debug, Clone)]
pub struct RadarChart {
    /// Data points (one per frame).
    pub points: Vec<RadarPoint>,
    /// Chart title.
    pub title: String,
}

/// A rhythm report summarizing attention patterns over time.
#[derive(Debug, Clone)]
pub struct RhythmReport {
    /// Report title.
    pub title: String,
    /// Period covered (e.g. "2026-06-15 to 2026-06-22").
    pub period: String,
    /// Total number of decisions in the period.
    pub total_decisions: usize,
    /// Number of Hold outcomes.
    pub hold_count: usize,
    /// Average ASI over the period.
    pub avg_asi: f64,
}
...
/// attention data, and export preferences.
#[derive(Debug, Clone)]
pub struct ViewState {
    ...
    /// Radar chart data.
    pub radar: Option<RadarChart>,
    /// Attention session data.
    pub attention: AttentionSession,
    /// Rhythm report (optional, for weekly/monthly views).
    pub rhythm: Option<RhythmReport>,
}
...
impl ViewState {
    ...
    /// Set the radar chart.
    pub fn set_radar(&mut self, chart: RadarChart) {
        self.radar = Some(chart);
    }

    /// Set the rhythm report.
    pub fn set_rhythm(&mut self, report: RhythmReport) {
        self.rhythm = Some(report);
    }
...
/// The single interface for rendering views.
pub trait RenderPort {
    /// Render the view state as an HTML string.
    fn render_html(&self, state: &ViewState) -> String;

    /// Render the view state as a JSON string.
    fn render_json(&self, state: &ViewState) -> Result<String, BcError>;
}
...
impl RenderPort for AuroraRenderer {
    fn render_html(&self, state: &ViewState) -> String {
...
mod tests {
    ...
    #[test]
    fn radar_chart_can_be_set() {
        let mut state = make_view_state();
        state.set_radar(RadarChart {
            points: vec![
                RadarPoint {
                    frame: "Embodied".into(),
                    phase: 0.8,
            ...
            title: "Attention Radar".into(),
        });

        assert!(state.radar.is_some());
        assert_eq!(state.radar.as_ref().unwrap().points.len(), 2);
    }

    #[test]
    fn rhythm_report_can_be_set() {
        let mut state = make_view_state();
        state.set_rhythm(RhythmReport {
            title: "Weekly Rhythm".into(),
            period: "2026-06-15 to 2026-06-22".into(),
            total_decisions: 42,
            hold_count: 7,
            avg_asi: 0.65,
        });

        assert!(state.rhythm.is_some());
        assert_eq!(state.rhythm.as_ref().unwrap().hold_count, 7);
    }
}
