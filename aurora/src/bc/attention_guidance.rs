...
//!
//! # Port
//! [`AttentionPort`] trait — the single interface for attention guidance.
//!
//! # Migration note
...
/// The single interface for attention guidance.
///
/// Runs the attention scheduling loop: feed signals → get reminders → record responses.
pub trait AttentionPort {
    ...
    fn run_cycle(&mut self, signals: &[TritWord]) -> Option<AttentionCmd>;

    /// Record the user's response to the last reminder.
    fn respond(&mut self, response: UserResponse);

    /// Current ASI value.
    fn asi(&self) -> f64;

    /// Access the underlying session.
    fn session(&self) -> &AttentionSession;
}
...
impl AttentionPort for AttentionManager {
    fn run_cycle(&mut self, signals: &[TritWord]) -> Option<AttentionCmd> {
