...
//!
//! # Engine
//! [`FftWaveletEngine`] — FFT-based analysis (wraps the wavelet module).
...
// ── Implementation (wraps existing wavelet module) ────────────────────────

/// FFT-based wavelet engine — wraps the existing wavelet-module implementation.
pub struct FftWaveletEngine;

impl FftWaveletEngine {
    /// Analyze a time series and return its frequency spectrum.
    pub fn analyze(&self, signal: &TimeSeries) -> Result<FrequencySpectrum, BcError> {
